        ...
        stage ('Deploy') {
            steps {
              withCredentials([file(credentialsId: 'deploy-server', variable: 'deployment')])  {
                   ...
                   sh 'ssh -v -i ${deployment} -o StrictHostKeyChecking=no ubuntu@13.233.251.211 "cd /home/ubuntu; unzip -o XFS.zip -d xfs_dashboard;pm2 start dist/XFS "serve -p 3000 ." --name "xfs_dashboard""'                  
               }
            }
        }
